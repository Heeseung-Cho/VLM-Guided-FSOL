...
are replaced by the FULL supercategory leaf categories of the declared route_type.
...
def main():
    ...
    ap.add_argument('--supercategory_config', required=True)
    args = ap.parse_args()
    ...
    for r in recs:
        ...
        sup_cats = get_supercategory_categories(route) or []
        if not sup_cats:
            continue
        # Preserve VLM-pick ordering, then append siblings (so VLM's confidence is rank-1)
        base_cats = r.get('semantic_categories', []) or []
        ordered = [c for c in base_cats if c in sup_cats]
        ordered += [c for c in sup_cats if c not in ordered]
        r['semantic_categories'] = ordered
        # Rebuild proposal_prompts: keep original cue parts that aren't category names,
        # plus all supercategory leaf category names.
        old_prompts = r.get('proposal_prompts', []) or []
        non_cat_prompts = [p for p in old_prompts if p not in sup_cats and p not in base_cats]
